<div style="display: none;" class="page" id="page-<?php echo $index ?>">
  <div class="part" id="part-<?php echo $index ?>">
    ...
    <p>
      ...
      <select id="part[<?php echo ($index-1) ?>][filter_id]" name="part[<?php echo ($index-1) ?>][filter_id]" onchange="$('markdown_toolbar_<?php echo ($index-1) ?>').style.display = this.options[this.selectedIndex].value == 'Markdown' ? 'block': 'none';">
...
<?php endforeach; ?>
    ...
    <script>
      markdown_toolbar = new Control.TextArea.ToolBar.Markdown('part[<?php echo ($index-1) ?>][content]');
      markdown_toolbar.toolbar.container.id = 'markdown_toolbar_<?php echo ($index-1) ?>';
      <?php if ($filter_id != 'Markdown') echo '$(\'markdown_toolbar_'.($index-1).'\').style.display = \'none\';'; ?>
    </script>
  </div>
</div>