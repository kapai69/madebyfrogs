    ...
    <li id="page_<?php echo $child->id; ?>" class="node level-<?php echo $level; if ( ! $child->has_children) echo ' no-children'; else if ($child->is_expanded) echo ' children-visible'; else echo ' children-hidden'; ?> dragable">
      ...
      <?php 
        switch ($child->status_id) {
          case PAGE_STATUS_DRAFT: echo '<div class="status draft-status">Draft</div>'; break;
          case PAGE_STATUS_REVIEWED: echo '<div class="status reviewed-status">Reviewed</div>'; break;
          case PAGE_STATUS_PUBLISHED: echo '<div class="status published-status">Published</div>'; break;
          case PAGE_STATUS_HIDDEN: echo '<div class="status hidden-status">Hidden</div>'; break;
        }
        ?> 
      <div class="modify">
        <a href="<?php echo get_url('pages/add', $child->id); ?>"><img alt="add child" src="images/add-child.png" /></a>
        <a class="remove" href="<?php echo get_url('pages/delete', $child->id); ?>"><img alt="remove page" src="images/remove.png" /></a>
        <img class="handle" src="images/drag.png" alt="drag and drop to reorder the list" />
      </div>
