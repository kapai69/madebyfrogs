...
<h1><?php echo __(Inflector::camelize($action).' Snippet') ?></h1>
...
<form action="<?php echo $action=='edit' ? get_url('snippet/edit/'.$snippet->id): get_url('snippet/add');  ?>" method="post">
  <div class="form-area">
    ...
    <p>
      <label for="snippet_filter"><?php echo __('Filter') ?></label>
      <select id="snippet[filter_id]" name="snippet[filter_id]" onchange="$('markdown_toolbar').style.display = this.options[this.selectedIndex].value == 'Markdown' ? 'block': 'none';">>
...
<?php if (isset($snippet->updated_on)) { ?>
    <p style="clear: left"><small><?php echo __('Last updated by') ?> <?php echo $snippet->updated_by_name ?> <?php echo __('on') ?> <?php echo $snippet->updated_on ?></small></p>
<?php } ?>
  </div>
  <p class="buttons">
...
// <![CDATA[
  markdown_toolbar = new Control.TextArea.ToolBar.Markdown('snippet_content');
  markdown_toolbar.toolbar.container.id = 'markdown_toolbar';
  <?php if ($snippet->filter_id != 'Markdown') echo '$(\'markdown_toolbar\').style.display = \'none\';'; ?>
