...
class PDOMysql
{
    ...
    {
        return new PDOStatementMysql($query, $this->_connection, $this->_dbinfo);
    }
    ...
    {
        if ($attribute === PDO::ATTR_PERSISTENT && $mixed != $this->_persistent) {
    ...
    private function _setErrors($error)
    {
        ...
        log_error('PDO error Code: '. $error);
        log_error('PDO error No: '. $errno);
        log_error('PDO error Info: '. $errst);
    }

} // End PDOMysqlDriver class



/**
 * PDOStatement adpater for MySQL
 *
 * For more specification on methods of this class check the PDO class
 * can be find on the same directory at PDO.php
 *
...
class PDOStatementMysql extends PDOStatement
{
    ...
    protected function _setErrors($error)
    {
        ...
        log_error('PDO error Code: '. $error);
        log_error('PDO error No: '. $errno);
        log_error('PDO error Infp: '. $errst);
    }
    
    protected function _query($query)
    {
        log_debug('PDO._uquery: '.$query);
